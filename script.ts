...
const keyValueTemplate: HTMLTemplateElement = document.querySelector("[data-key-value-template]");
...
document.querySelector("[data-form]").addEventListener("submit", (e) => {
  ...
  axios({
    ...
    method: <Method>document.querySelector<HTMLSelectElement>("[data-method]").value,
...
function createKeyValuePair() {
  ...
  const element = <Element>keyValueTemplate.content.cloneNode(true);
  // const element = keyValueTemplate.content;
  //Add an event listener to the remove button
  element.querySelector("[data-remove-btn]").addEventListener("click", (e: Event) => {
    (e.target as Element).closest("[data-key-value-pair]").remove();
  });
  return element;
}

//Function to return the params and headers as objects
function keyValuePairsToObjects(container: Element) {
  const pairs = container.querySelectorAll("[data-key-value-pair]");
  //Convert all in a array
  return [...pairs].reduce((data, pair) => {
    //Get the data of each pair
    const key = pair.querySelector<HTMLInputElement>("[data-key]").value;
    const value = pair.querySelector<HTMLInputElement>("[data-value]").value;
...
function updateEndTime(response: CustomAxiosResponse) {
  response.customData = response.customData || {};
  response.customData.time = new Date().getTime() - response.config.customData.startTime;
  return response;
}

function showResponse(response: CustomAxiosResponse) {
...
function updateResponseDetails(response: CustomAxiosResponse) {
...
function updateResponseHeaders(headers: Record<string, string>) {
